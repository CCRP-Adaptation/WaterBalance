...
DailyClimData = read.csv("C:\\David\\Water balance\\Operational version\\Daymet Batch Example Penman Hamon\\R code\\test\\Daymet\\1.csv_1980_2018.csv", skip=7, header=TRUE)
head(DailyClimData)
DailyClimData$Date = as.Date(DailyClimData$Date, "%m/%d/%Y")

#if using Daymet climate data need to do some formatting
#add date column calculated from year and day of year
Date<-as.Date(DailyClimData$yday-1, origin=paste(DailyClimData$year,"-01-01", sep=""))#-1 needed b/c R uses zero based index
DailyClimData<-cbind(Date,DailyClimData);head(DailyClimData)

#calculate average temperature and rename columns to match what's expected by code below
tmean_C <- rowMeans(DailyClimData[8:9])
DailyClimData<-cbind(DailyClimData,tmean_C);head(DailyClimData)

#rename columns
names(DailyClimData)[]<-c("Date","year","yday","dayl","ppt_mm","srad","swe","tmax","tmin","vp","tmean_C");head(DailyClimData)
...
#Output directory
OutDir = "C:\\David\\Water balance\\Operational version\\Daymet Batch Example Penman Hamon\\R code\\test"
